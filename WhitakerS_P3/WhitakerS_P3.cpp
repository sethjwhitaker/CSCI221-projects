...
// Constructor for Fraction class. Sets num to a and den to b.
Fraction::Fraction(int a, int b) {

	if (b < 0) { // If den is negative, set num to be negative and den to be positive
...
// Constructor for Fraction class. Sets num to a and den to 1.
...
// Default constructor for Fraction class. Sets num to 0 and den to 1.
Fraction::Fraction() {
...
// Accessor method for the num variable of the Fraction class.
...
// Accessor method for the den variable of the Fraction class.
...
// Method to reduce fraction to lowest form.
void Fraction::reduce() {

	int n = gcd(); // Find greatest common denominator

	num /= n;      // Divide the numerator by the gcd
	den /= n;      // Divide the denominator by the gcd

}

// Method that returns a new Fraction with the numerator and denominator swapped.
...
// Overloaded addition operator that returns the reduced sum of f1 and f2.
Fraction operator +(const Fraction& f1, const Fraction& f2) {

	// For addition of fractions, both fractions must have the same denominator.
	// Therefore, we multiply the num by the den of the other, and we multiply the dens together
	int newNum1, newNum2, newDen;

	newNum1 = f2.den * f1.num;	
	newNum2 = f1.den * f2.num;
	newDen = f1.den * f2.den;

	// Create a new Fraction using the sum of the new nums of f1 and f2 and the new common den
	Fraction newF = Fraction(newNum1 + newNum2, newDen); 

	newF.reduce();	// Reduce the new fraction to lowest terms

	return newF;

}

// Overloaded subtraction operator that returns the reduced difference between f1 and f2.
Fraction operator -(const Fraction& f1, const Fraction& f2) {

	// For subtraction of fractions, both fractions must have the same denominator.
	// Therefore, we multiply the num by the den of the other, and we multiply the dens together
	int newNum1, newNum2, newDen;
	...
	// Create a new Fraction using the difference between the new nums of f1 and f2 and the new common den
	Fraction newF = Fraction(newNum1 - newNum2, newDen);
...
// Overloaded multiplication operator that returns the product of f1 and f2.
Fraction operator *(const Fraction& f1, const Fraction& f2) {

	// Create a new Fraction using the product of the nums and dens of f1 and f2.
	Fraction newF = Fraction(f1.num * f2.num, f1.den * f2.den);
...
// Overloaded division operator that returns the quotient of f1 divided by f2
Fraction operator /(const Fraction& f1, const Fraction& f2) {

	// Create a new Fraction that contains the reciprocal of f2
	Fraction r = f2.reciprocal();

	// Create a new Fraction using the product of the nums and dens of f1 and the reciprocal of f2.
	Fraction newF = Fraction(f1.num * r.num, f1.den * r.den);
...
// Overloaded negation operator that returns the negation of f.
Fraction operator -(const Fraction& f) {

	// Return a new fraction with negated num and the same den.
	return Fraction(-f.num, f.den);

}

// Overloaded less than operator that returns true if f1 < f2 and false otherwise.
bool operator < (const Fraction& f1, const Fraction& f2) {

	// Get the nums of the fractions when the dens are equal to each other
	int newNum1, newNum2;
...
// Overloaded greater than operator that returns true if f1 > f2 and false otherwise.
bool operator > (const Fraction& f1, const Fraction& f2) {
...
// Overloaded less than or equal to operator that returns true when f1 <= f2 
// and false otherwise
bool operator <= (const Fraction& f1, const Fraction& f2) {
...
// Overloaded greater than or equal to operator that returns true when f1 <= f2 
// and false otherwise
bool operator >= (const Fraction& f1, const Fraction& f2) {
...
// Overloaded equal to operator that returns true when f1 == f2 
// and false otherwise
bool operator == (const Fraction& f1, const Fraction& f2) {
...
// Overloaded not equal to operator that returns true when f1 != f2 
// and false otherwise
bool operator != (const Fraction& f1, const Fraction& f2) {
...
// Overloaded extraction operator that inputs a fraction from istream in into Fraction f
istream& operator >> (istream& in, Fraction& f) {

	int a, b; // a will hold numerator and b will hold denominator

	in >> a;
	in.get(); // ignore the '/' character
	in >> b;
...
// Overloaded insertion operator that outputs a fraction to ostream
...
******\WhitakerS_P3.exe (process 20496) exited with code 0.
